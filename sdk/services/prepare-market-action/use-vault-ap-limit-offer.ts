...
export const vaultAPLimitOfferSchema = z.object({
  quantity: z
    .string()
...
export const calculateVaultAPLimitOfferTokenData = ({
  ...
  /**
   * @note Here, incentive_token_raw_amount and all it's related values represent the rate of incentive tokens per second
   */
  ...
  incentiveData = action_incentive_token_ids.map(
    (incentive_token_id, index) => {
      ...
      const scaled_incentive_token_per_second_per_input_token = parseRawAmount(
        tokenAmounts[index] ?? "0",
      );

      const incentive_token_per_second = BigNumber.from(
        scaled_incentive_token_per_second_per_input_token,
      )
        .mul(BigNumber.from(quantity ?? "0"))
        .div(BigNumber.from(10).pow(18))
        .toString();

      // Get incentive token raw amount
      const incentive_token_raw_amount: string = incentive_token_per_second;
      ...
      if (quantity_value_usd > 0) {
        annual_change_ratio =
          (incentive_value_usd / quantity_value_usd) * (365 * 24 * 60 * 60);
      }
...
export const getVaultAPLimitOfferCompletionStatus = ({
  quantity,
  walletTokenBalance,
  vaultTokenBalance,
  completeMarket,
...
export const useVaultAPLimitOffer = ({
  ...
  const { inputTokenData, incentiveData } = calculateVaultAPLimitOfferTokenData(
    {
  ...
  let { canBePerformedCompletely, canBePerformedPartially } =
    getVaultAPLimitOfferCompletionStatus({
      quantity: quantity ?? "0",
      walletTokenBalance: propsWalletTokenBalance.data ?? null,
      vaultTokenBalance: propsVaultTokenBalance.data ?? null,
      completeMarket: propsCompleteMarket.data ?? null,

...
export type GetReadMarketQueryParams = {
  chain_id: number;
  market_type: number;
  market_id: string;
};

export type GetReadMarketQueryOptionsParams = GetReadMarketQueryParams & {
  RPC_API_KEYS?: TypedRpcApiKeys;
};

export const getReadMarketQueryFunction = async ({
  chain_id,
  market_type,
  market_id,
  RPC_API_KEYS,
}: GetReadMarketQueryOptionsParams) => {

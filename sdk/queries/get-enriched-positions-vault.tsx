...
export const constructEnrichedPositionsVaultFilterClauses = (
...
export type GetEnrichedPositionsVaultQueryParams = {
...
export type GetEnrichedPositionsVaultQueryOptionsParams =
  GetEnrichedPositionsVaultQueryParams & {
    client: TypedRoycoClient;
    RPC_API_KEYS: TypedRpcApiKeys;
  };

export const getEnrichedPositionsVaultQueryFunction = async ({
  client,
  RPC_API_KEYS,
  account_address,
  chain_id,
  ...
  if (!!result.data && !!result.data.data && result.data.data.length > 0) {
          ...
          const tokens_data = row.token_ids.map((tokenId, tokenIndex) => {
            ...
            if (row.offer_side === RoycoMarketUserType.ap.value) {
              raw_amount = parseRawAmount(
                raw_incentives_amount_ap_data[tokenIndex]?.result?.toString() ??
                  "0",
              );
            } else {
              raw_amount = parseRawAmount(
    ...
    const new_rows = updated_rows.filter((row) => !!row);

    return {
      count: result.data.count ?? 0,
      data: new_rows as Array<EnrichedPositionsVaultDataType> | null,
    };
  }

  return {
    count: 0,
    data: null as Array<EnrichedPositionsVaultDataType> | null,
  };
};
...
export const getEnrichedPositionsVaultQueryOptions = ({
  ...
  queryFn: () =>
    getEnrichedPositionsVaultQueryFunction({
      client,
      RPC_API_KEYS,
      account_address,
      chain_id,
